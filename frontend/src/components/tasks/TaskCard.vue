<template>
  <div class="bg-white/80 backdrop-blur-sm rounded-2xl border border-gray-200/60 shadow-sm hover:shadow-lg transition-all duration-300 transform hover:scale-[1.02] group"
       :class="{
    ...
    <div class="p-4 border-b border-gray-200/60">
      <div class="flex items-start justify-between">
        ...
        <div class="flex-1 min-w-0">
          ...
          <div v-if="task.tags && task.tags.length > 0" class="flex flex-wrap gap-1 mb-2">
            ...
            <span v-if="task.tags.length > 3" class="inline-flex items-center px-2 py-1 rounded-lg text-xs bg-gray-100 text-gray-600 font-medium">
              +{{ task.tags.length - 3 }}
            </span>
          </div>
        </div>
        ...
        <div class="relative">
          <button @click="showActions = !showActions"
                  class="p-2 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors duration-200 opacity-0 group-hover:opacity-100">
          ...
          <!-- Dropdown Menu -->
          <div v-if="showActions" class="absolute right-0 top-full mt-1 w-48 bg-white/95 backdrop-blur-xl rounded-xl shadow-lg border border-gray-200/60 py-1 z-10">
            <!-- Edit Button -->
            <button @click="handleEdit" class="flex items-center space-x-2 w-full px-4 py-2 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition-colors duration-200">
            ...
            <!-- Delete Button -->
            <button @click="handleDelete" class="flex items-center space-x-2 w-full px-4 py-2 text-sm text-red-600 hover:bg-red-50 transition-colors duration-200">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <!-- Description -->
      <p v-if="task.description" class="text-sm text-gray-600 line-clamp-2 mt-2">
        {{ task.description }}
      </p>
    </div>
    ...
    <div class="p-4">
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center space-x-2">
          ...
          <span class="text-sm" :class="dueDateClasses">
            {{ formattedDueDate }}
          </span>
        </div>
        ...
        <div v-if="task.assignedTo" class="flex items-center space-x-2">
          <div class="w-6 h-6 bg-gradient-to-r from-blue-500 to-purple-600 rounded-full flex items-center justify-center text-xs text-white font-medium">
            {{ task.assignedTo.username?.charAt(0)?.toUpperCase() || 'U' }}
          </div>
          <span class="text-sm text-gray-600">{{ task.assignedTo.username || 'مستخدم' }}</span>
        </div>
      </div>
      ...
      <div class="flex items-center justify-between mt-3">
        ...
        <button @click="toggleCompletion"
                class="flex items-center space-x-2 px-3 py-2 rounded-xl text-sm font-medium transition-all duration-200"
                :class="task.completed ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path v-if="task.completed" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
            <path v-else stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
          </svg>
          <span>{{ task.completed ? 'مكتملة' : 'تعيين كمكتملة' }}</span>
        </button>

        <!-- Created Info -->
        <div class="text-xs text-gray-500">
          أنشئت {{ formatCreatedAt }}
        </div>
      </div>
...
const formatCreatedAt = computed(() => {
  if (!props.task.createdAt) return 'قبل فترة'
  return formatDistanceToNow(new Date(props.task.createdAt), { addSuffix: true })
})
